...
for src, what, when in STRIP:
    band.append(
        ...
        '        <span style="position: absolute; left: 0; right: 0; bottom: 92px; display: flex; flex-direction: column; gap: 2px; padding: 0 8px;">\n'
...
si.append(
    '  <div style="flex: none; margin-top: -24px; padding: 0 16px;">\n'
    '    <h1 style="margin: 0; font-size: 36px; font-weight: 700; line-height: 1.2; letter-spacing: -0.03em;">Say what you did.<br>Show it if it matters.</h1>\n'
    '    <p style="margin: 12px 0 0; font-size: 15.5px; line-height: 1.45; color: ' + GREY + ';">A tracker with your friends watching. You pick the windows. Miss one and it costs you.</p>\n'
    '  </div>\n')
...
si.append('  <div style="flex: none; margin: 24px 16px 0; display: flex; flex-direction: column;">\n')
for i, (n, head, body) in enumerate(HOW):
    top = 'transparent' if i == 0 else SEP
    si.append(
        '    <div style="display: flex; gap: 12px; padding: 12px 0; border-top: 0.5px solid ' + top + ';">\n'
        ...
        '        <span style="font-size: 12.5px; line-height: 1.45; color: ' + GREY + ';">' + body + '</span>\n'
        '      </span>\n'
        '    </div>\n')
si.append('  </div>\n')
...
si.append(grow())
si.append(
    '  <div style="flex: none; padding: 0 16px 32px; display: flex; flex-direction: column; gap: 12px;">\n'
    '    <button type="button" style="width: 100%; height: 54px; border: 0; border-radius: 15px; background: #ffffff; color: #000000; font-family: inherit; font-size: 17px; font-weight: 600; display: flex; align-items: center; justify-content: center; gap: 8px;">\n'
...
mk.append('  <div style="flex: none; margin: 20px 16px 0; height: 196px; border-radius: 18px; background: #0d0d0f; display: flex; align-items: center; justify-content: center;">'
          + mark(112) + '</div>\n')

# Construction, drawn rather than described.
mk.append(section('How it is built', top=28))
mk.append('  <div style="flex: none; margin: 12px 16px 0; border-radius: 16px; background: #0d0d0f; padding: 20px; display: flex; flex-direction: column; align-items: center; gap: 16px;">\n')
...
''')
mk.append('    <div style="display: flex; flex-wrap: wrap; gap: 4px; justify-content: center;">\n')
for spec in ('32 GRID', '2 MARGIN', '13 SQUARE', '2 GUTTER', '0 RADIUS', 'CRISP EDGES'):
    mk.append('      <span style="padding: 4px 8px; border-radius: 7px; background: ' + CARD2 + '; font-family: ' + MONO
              + '; font-size: 9.5px; font-weight: 600; letter-spacing: 0.08em; color: ' + GREY + ';">' + spec + '</span>\n')
mk.append('    </div>\n  </div>\n')
mk.append('  <p style="flex: none; margin: 12px 16px 0; font-size: 12.5px; line-height: 1.45; color: ' + GREY
...
mk.append('  <div style="flex: none; margin: 12px 16px 0; border-radius: 16px; background: #0d0d0f; padding: 20px 16px 16px; display: flex; align-items: flex-end; justify-content: space-between;">\n')
for size in (16, 20, 24, 32, 48, 64):
    mk.append('    <div style="display: flex; flex-direction: column; align-items: center; gap: 8px;">' + mark(size)
...
mk.append('  <p style="flex: none; margin: 8px 16px 0; font-size: 12.5px; line-height: 1.45; color: ' + GREY
...
for fam, wt, tr, label, note, live in WORDS:
    mk.append('  <div style="flex: none; margin: 12px 16px 0; border-radius: 14px; background: #0d0d0f; border: 1px solid '
              + ('rgba(255,55,95,0.4)' if live else 'transparent')
              + '; padding: 16px 16px 16px; display: flex; flex-direction: column; gap: 12px;">\n'
              ...
              '      <span style="flex-grow: 1; font-size: 11.5px; line-height: 1.35; color: ' + GREY + ';">' + note + '</span>\n'
              '    </div>\n'
              '  </div>\n')
...
mk.append('  <div style="flex: none; margin: 12px 16px 0; border-radius: 16px; background: #0d0d0f; padding: 24px 16px 20px; display: flex; flex-direction: column; align-items: center; gap: 20px;">\n'
          '    <div>' + lava_mark(96, 'lvA') + '</div>\n'
          '    <div style="width: 100%; display: flex; gap: 16px; align-items: flex-end; justify-content: center;">\n'
          '      <div style="display: flex; flex-direction: column; align-items: center; gap: 8px;">' + lava_mark(64, 'lvB') + '<span style="font-family: ' + MONO + '; font-size: 9.5px; font-weight: 700; letter-spacing: 0.1em; color: ' + GREY + ';">SIGN IN</span></div>\n'
          '      <div style="display: flex; flex-direction: column; align-items: center; gap: 8px;">' + lava_mark(32, 'lvC') + '<span style="font-family: ' + MONO + '; font-size: 9.5px; font-weight: 700; letter-spacing: 0.1em; color: ' + GREY + ';">SPLASH</span></div>\n'
          '    </div>\n'
          '  </div>\n')
mk.append('  <p style="flex: none; margin: 12px 16px 0; font-size: 12.5px; line-height: 1.45; color: ' + GREY + ';">It fills once, on arrival, and stops a sliver short of the top: a solid block is not a window onto anything. What keeps moving after that is the surface and the churn beneath it, because a logo that empties and refills is a loading spinner.</p>\n')
mk.append('  <p style="flex: none; margin: 12px 16px 0; font-size: 12.5px; line-height: 1.45; color: ' + GREY + ';"><b style="color:#ffffff;font-weight:600;">Two places only: sign in and the splash.</b> Everywhere inside the app the mark is flat and white. This is the moment somebody first meets Curfew, and a brand gets one of those.</p>\n')

# The exploration the answer asked for, labelled as exploration.
mk.append(section('If it carried your streak', top=28))
mk.append('  <div style="flex: none; margin: 8px 16px 0; border-radius: 12px; background: rgba(255,159,10,0.1); border: 1px solid rgba(255,159,10,0.32); padding: 12px 12px; display: flex; gap: 12px;">\n'
          '    <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="' + AMBER + '" stroke-width="2.2" stroke-linecap="round" aria-hidden="true" style="flex: none; margin-top: 2px;"><circle cx="12" cy="12" r="9"/><path d="M12 11v5M12 7.5v.01"/></svg>\n'
...
for i in range(0, 6, 2):
    mk.append('  <div style="flex: none; margin: 12px 16px 0; display: flex; gap: 12px; align-items: flex-start;">\n')
...
mk.append('  <p style="flex: none; margin: 20px 16px 0; font-size: 12.5px; line-height: 1.45; color: ' + GREY
