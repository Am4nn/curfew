...
deployment against the APAC database; `git tag vX.Y.Z && git push origin vX.Y.Z`
runs `.github/workflows/deploy.yml`, which typechecks, tests, then deploys with
the Vercel CLI. Vercel's production branch is `production`, which nobody pushes.
