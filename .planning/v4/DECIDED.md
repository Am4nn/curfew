...
**Not yet settled:** whether the RECORD stays clerk. `DIRECTION.md` §2 proposes
three registers by surface, with every number, time, money amount and ledger row
staying flat and unadjectived, on the argument that this is the app's
credibility and is not for sale. That proposal is neither overturned nor
confirmed. See 3.1.
...
the old design. The gate has to be re-pointed at v5 or retired, and until it is,
two files in this repo both claim to be the spec.
...
### 3.1 Does the record stay clerk?

`DIRECTION.md` §2 argues that numbers, times, money and the ledger keep the flat
register even in a coach app, because that is the app's credibility. Aman's
decision was "coach app" without ruling on this. Both readings are live:

- **Three registers.** The coach is warm, the ledger is not. A fine still reads
  "₹50, settled".
- **Coach everywhere.** The register is the app, including its numbers.

The structural rule from `notification-copy.ts` is worth keeping under either
reading: **a writer in the warm register never receives the numbers**, so it
cannot claim progress it has not measured. That rule was bought with a real
defect, when production told somebody with zero of eight glasses that they were
almost there, twice, half an hour apart.

### 3.2 What happens to the clerk copy shipped this week

3.4.3 and 3.4.4 shipped grey-streak strings in pure clerk. *"This week can no
longer be made."* Those are v4 rework. Fine, but nobody should be surprised.

### 3.3 Which model, where it runs, and who pays
...
### 3.4 How a nudge is rate limited, and whether it can be refused
...
and a nudge has to respect them or that setting becomes a lie. Open: a per-sender
cap, a per-recipient cap, whether a member can turn nudges off from one specific
person, and whether the recipient sees who sent it.

### 3.5 Whether ROADMAP theme 1 still comes first

ROADMAP says the per-type review precedes all of this, because *"a redesign of
screens whose rules are wrong just makes the wrong rules prettier."* With one
release that argument either folds into v4 or gets dropped. It has not been
dropped on purpose yet.

### 3.6 The dopamine detox activity
...
### 3.7 The three review items the canvas notes still carry
