...
const CAPS: Record<Role, Capability[]> = {
  ...
  moderator: ["users.view", "users.approve", "groups.view", "ledger.view"],
  // Ops runs the derivable jobs and reads data, but cannot approve or set roles.
  ops: ["users.view", "groups.view", "ledger.view", "ops.score", "ops.verify"],
  // Auditor is read-only across the console.
  auditor: ["users.view", "groups.view", "ledger.view"],
