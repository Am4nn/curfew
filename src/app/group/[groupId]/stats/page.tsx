    ...
    <main className="min-h-dvh pb-nav">
      <header className="flex items-center justify-between gap-3 border-b border-rule px-5 pb-[11px] pt-5">
        <div className="flex items-center gap-[9px]">
          <BackLink fallback={`/group/${groupId}`} className="text-base text-muted" />
          <span className="text-base font-semibold tracking-caps">GROUP STATS</span>
        </div>
        ...
        {week.of === 0 ? (
          ...
          <>
            <section className="flex flex-col gap-1.5">
              <span className="text-micro tracking-label text-muted">THIS WEEK</span>
              <div className="flex items-baseline gap-2.5">
                <span className="text-4xl font-semibold leading-none tabular-nums">
                  {week.done}
                </span>
                <span className="text-base text-muted">of {week.of} done</span>
              </div>
            ...
            {week.away.length > 0 ? (
              <div className="flex flex-col gap-1 border border-accent p-3">
            ...
            <section className="flex flex-col gap-2.5">
              <span className="text-micro tracking-label text-muted">DAY BY DAY</span>
              <div className="flex gap-[5px]">
                {week.byDay.map((d) => (
                  <div key={d.day} className="flex flex-1 flex-col items-center gap-1.5">
                    ...
                    <span
                      className={
                        "text-micro tabular-nums " + (d.away > 0 ? "text-accent" : "text-muted")
                      }
                    >
                      {d.done}
                    </span>
              ...
              <div className="flex gap-[5px]">
                {week.byDay.map((d) => (
                  <span key={d.day} className="flex-1 text-center text-micro text-muted">
                  ...
                  <div key={m.name} className="flex items-center gap-[11px] border-b border-rule py-[11px]">
                    <div className="flex flex-1 flex-col gap-1.5">
                      <div className="flex items-center justify-between gap-[9px]">
                        ...
                        <span
                          className={
                            "flex-none text-2xs tabular-nums " +
            ...
            <section className="flex flex-col gap-2.5">
              <span className="text-micro tracking-label text-muted">BY ACTIVITY</span>
              <div className="flex flex-col">
                {week.byType.map((t) => (
