...
def rows_card(rows, margin=14):
    ...
    for i, r in enumerate(rows):
        ...
        inner = ("""      <span style="flex-grow: 1; min-width: 0; display: flex; align-items: center; gap: 12px; padding: 12px 16px 12px 0; border-top: 0.5px solid %s;">
...
# opens twice.
st.append("""  <div style="flex: none; padding: 4px 16px 0; display: flex; align-items: center; gap: 16px;">
...
st.append("""  <div style="flex: none; margin: 12px 16px 0; border-radius: 12px; background: %s; padding: 4px; display: flex; gap: 4px;">
    <sc-for list="{{themes}}" as="t" hint-placeholder-count="3">
      <button type="button" onClick="{{t.pick}}" aria-pressed="{{t.onStr}}" style="flex-grow: 1; flex-basis: 0; height: 38px; border: 0; border-radius: 9px; background: {{t.bg}}; color: {{t.fg}}; font-family: inherit; font-size: 14.5px; font-weight: {{t.weight}}; display: flex; align-items: center; justify-content: center; gap: 8px;">
...
st.append('  <p style="flex: none; margin: 8px 16px 0; font-size: 12.5px; line-height: 1.45; color: %s;">Every window and fine is judged in this zone.</p>\n' % GREY)
...
st.append("""  <div style="flex: none; margin: 24px 16px 0; border-radius: 14px; background: %s; overflow: hidden;">
    <button type="button" style="width: 100%%; border: 0; background: transparent; padding: 16px 16px; text-align: left; font-family: inherit; font-size: 16px; color: %s;">Sign out</button>
  </div>
""" % (CARD, RED))
st.append(grow())
...
sh.append("""  <!--
  ...
  -->
  <div style="flex: none; margin: 20px 16px 0; border-radius: 14px; background: %s; padding: 16px 16px; display: flex; flex-direction: column; gap: 8px;">
    <div style="display: flex; align-items: baseline; gap: 8px;">
      <span style="font-family: %s; font-size: 11px; font-weight: 700; letter-spacing: 0.1em; color: %s;">WHAT SHARING BUYS</span>
    </div>
    <span style="font-size: 13.5px; line-height: 1.45; color: %s;">Your ceiling is the share of a group you share. <span style="color:#ffffff;">Four of five in Wing caps you at 850.</span> Your score settles down, two points a day.</span>
  </div>
""" % (CARD, MONO, GREY, GREY))

for gname, gnote, gkey in GROUPS:
    sh.append("""  <div style="flex: none; margin: 24px 16px 0;">
    <div style="display: flex; align-items: baseline; gap: 8px;">
...
sh.append("""  <p style="flex: none; margin: 20px 16px 0; font-size: 12.5px; line-height: 1.45; color: %s;">Stops now and takes back what was sent. They have already seen them.</p>
""" % GREY)
sh.append(grow())
...
nt.append("""  <div style="flex: none; margin: 20px 16px 0; border-radius: 14px; background: %s; overflow: hidden;">
    ...
    <div style="display: flex; align-items: center; gap: 12px; padding: 12px 16px; border-top: 0.5px solid %s;">
      <span style="flex-grow: 1; min-width: 0; display: flex; flex-direction: column; gap: 2px;">
      ...
      <span style="flex: none; display: flex; gap: 4px;">
        <span style="padding: 4px 8px; border-radius: 8px; background: %s; font-family: %s; font-size: 14px; font-weight: 500;">10:30 PM</span>
        <span style="padding: 4px 8px; border-radius: 8px; background: %s; font-family: %s; font-size: 14px; font-weight: 500;">7:00 AM</span>
      </span>
    </div>
  </div>
...
nt.append("""  <p style="flex: none; margin: 8px 16px 0; font-size: 13px; line-height: 1.45; color: %s;">Before a window shuts. Three a day at most.</p>
...
nt.append("""  <div style="flex: none; margin: 12px 16px 0; border-radius: 16px; background: #161618; border: 0.5px solid rgba(255,255,255,0.09); padding: 12px 16px; display: flex; gap: 12px;">
    ...
    <div style="flex-grow: 1; min-width: 0; display: flex; flex-direction: column; gap: 2px;">
      ...
      <span style="font-size: 14px; line-height: 1.35; color: #c7c7cc;">Reading shuts at 11:59 PM. Rahul and Priya already logged theirs.</span>
    </div>
  </div>
  <div style="flex: none; margin: 12px 16px 0;">
  ...
  <p style="flex: none; margin: 12px 16px 0; font-size: 12.5px; line-height: 1.45; color: %s;">Progress comes from the activity, in its own words.</p>
""" % (PINK, MONO, DIM, GREY))
nt.append(grow())
...
ph.append("""  <div style="flex: none; margin: 20px 16px 0; border-radius: 14px; background: %s; padding: 12px 16px; display: flex; gap: 12px;">
    <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="%s" stroke-width="2" stroke-linecap="round" aria-hidden="true" style="flex: none; margin-top: 2px;"><circle cx="12" cy="12" r="9"/><path d="M12 7.6v5l3 2"/></svg>
    <span style="flex-grow: 1; font-size: 13.5px; line-height: 1.45; color: %s;">Deleted 90 days after it was taken.</span>
  </div>
""" % (CARD, ORANGE, GREY))
...
for heading, items in GRID:
    ...
    ph.append('  <div style="flex: none; margin: 8px 16px 0; display: flex; flex-wrap: wrap; gap: 8px;">\n')
...
dt.append("""  <div style="flex: none; margin: 12px 16px 0; border-radius: 14px; background: %s; overflow: hidden;">
...
for i, (label, note, tone) in enumerate(DEL):
    ...
    dt.append("""    <button type="button" style="width: 100%%; border: 0; background: transparent; padding: 0 0 0 16px; display: flex; font-family: inherit; text-align: left;">
      <span style="flex-grow: 1; min-width: 0; display: flex; flex-direction: column; gap: 2px; padding: 12px 16px 12px 0; border-top: 0.5px solid %s;">
...
dt.append("""  <div style="flex: none; margin: 20px 16px 0; border-radius: 14px; background: %s; padding: 16px 16px; display: flex; gap: 12px;">
    <svg width="19" height="19" viewBox="0 0 24 24" fill="none" stroke="%s" stroke-width="1.9" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true" style="flex: none; margin-top: 2px;"><path d="M12 2v20M17 6.5C17 4.6 14.8 3.5 12 3.5S7 4.6 7 6.5 9.2 10 12 11s5 2.3 5 4.2-2.2 3.3-5 3.3-5-1.4-5-3.3"/></svg>
  ...
  <div style="flex: none; margin: 20px 16px 0;">
...
aw.append("""  <div style="flex: none; margin: 20px 16px 0; border-radius: 16px; background: %s; padding: 16px 16px; display: flex; align-items: center; gap: 16px;">
    <div style="flex: none; display: flex; flex-direction: column; align-items: center;">
      ...
      <span style="margin-top: 2px; font-family: %s; font-size: 10.5px; font-weight: 600; letter-spacing: 0.08em; color: %s;">LEFT</span>
    </div>
    <div style="flex-grow: 1; display: flex; flex-direction: column; gap: 4px;">
...
aw.append("""  <div style="flex: none; margin: 12px 16px 0; border-radius: 14px; background: %s; overflow: hidden;">
    <sc-for list="{{trips}}" as="t" hint-placeholder-count="2">
      <div style="padding-left: 16px;">
        <div style="display: flex; align-items: center; gap: 12px; padding: 12px 16px 12px 0; border-top: 0.5px solid {{t.sep}};">
...
aw.append("""  <div style="flex: none; margin: 12px 16px 0; display: flex; flex-direction: column; gap: 12px;">
  ...
  <p style="flex: none; margin: 16px 16px 0; font-size: 12.5px; line-height: 1.45; color: %s;">Your group sees that you are away.</p>
""" % GREY)
aw.append(grow())
...
ad = [HEAD, root(H)]
ad.append("""
  <div style="flex: none; height: 44px; padding: 0 16px; display: flex; align-items: center; gap: 8px;">
...
ad.append("""  <div style="flex: none; margin: 12px 16px 0; border-radius: 14px; background: %s; overflow: hidden;">
    <sc-for list="{{jobs}}" as="j" hint-placeholder-count="3">
      <div style="padding-left: 16px;">
        <div style="display: flex; align-items: center; gap: 12px; padding: 12px 16px 12px 0; border-top: 0.5px solid {{j.sep}};">
  ...
  <p style="flex: none; margin: 12px 16px 0; font-size: 12.5px; line-height: 1.45; color: %s;">Heartbeats, not deliveries.</p>
""" % (CARD, MONO, DIM, MONO, GREY, GREY))

ad.append(eyebrow('FAILURES, LAST 7 DAYS', GREY, top=26))
ad.append("""  <div style="flex: none; margin: 12px 16px 0; border-radius: 14px; background: %s; padding: 16px; display: flex; align-items: center; gap: 12px;">
...
ad.append("""  <div style="flex: none; margin: 12px 16px 0; border-radius: 14px; background: %s; overflow: hidden;">
  ...
  <div style="flex: none; margin: 12px 16px 0; height: 6px; border-radius: 999px; background: %s; position: relative; overflow: hidden;">
    <span class="fill" style="position: absolute; left: 0; top: 0; bottom: 0; width: 41%%; border-radius: 999px; background: %s;"></span>
  </div>
  <p style="flex: none; margin: 8px 16px 0; font-size: 12.5px; line-height: 1.45; color: %s;">Above the ceiling, Ask Ren stops. Nightly lines carry on.</p>
""" % (CARD, GREY, MONO, CARD2, PINK, GREY))

ad.append(eyebrow('EVIDENCE', GREY, top=26))
ad.append("""  <div style="flex: none; margin: 12px 16px 0; display: flex; gap: 8px;">
...
ad.append("""  <div style="flex: none; margin: 12px 16px 0; border-radius: 14px; background: %s; overflow: hidden;">
  ...
  <p style="flex: none; margin: 12px 16px 0; font-size: 12.5px; line-height: 1.45; color: %s;">Takes effect now. Rewrites nothing.</p>
""" % (CARD, GREY, GREY))
ad.append(grow())
